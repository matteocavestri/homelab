#!/bin/bash
##      .SYNOPSIS
##      Grafana Dashboard for Cloudflare - Using RestAPI to InfluxDB v2 Script
##
##      .DESCRIPTION
...
##      Original Author jorgedlcruz
##      Edited by Pokertour
##      Edited by Matteo Cavestri

##
# Configurations
##
# Endpoint URL for InfluxDB
InfluxDBURL="" #Your InfluxDB Server, http://FQDN or https://FQDN if using SSL
InfluxDBPort="" #Default Port
InfluxBucket="" #Default Database
InfluxOrg="" # Default Organization
InfluxToken="" # InfluxDB Token

# Endpoint URL for login action
cloudflareauthmethod="APIKEY" #Choose Authentication method. Either TOKEN (single token with specific rights) or APIKEY (Global API KEY with Email)
cloudflareapikey="" # Used in APIKEY Mode
cloudflareemail="" # Used in APIKEY Mode
cloudflarezone="" # Used in both Mode
cloudflareapitoken="" # Used in TOKEN Mode
...
;;
    ...
    for requests in $(echo "$cloudflareUrl" | jq -r '.data.viewer.zones[0].httpRequests1dGroups[].sum.requests'); do
        ...
        curl -i -XPOST "$InfluxDBURL:$InfluxDBPort/api/v2/write?org=$InfluxOrg&bucket=$InfluxBucket&precision=s" -H "Authorization: Token $InfluxToken" --data-binary "cloudflare_analytics,cfZone=$cloudflarezone cfRequestsAll=$cfRequestsAll,cfRequestsCached=$cfRequestsCached,cfRequestsUncached=$cfRequestsUncached,cfBandwidthAll=$cfBandwidthAll,cfBandwidthCached=$cfBandwidthCached,cfBandwidthUncached=$cfBandwidthUncached,cfThreatsAll=$cfThreatsAll,cfPageviewsAll=$cfPageviewsAll,cfUniquesAll=$cfUniquesAll $cfTimeStamp"
        ...
        for requests in $(echo "$cloudflareUrl" | jq -r '.data.viewer.zones[0].httpRequests1dGroups[].sum.countryMap[]'); do
            ...
            curl -i -XPOST "$InfluxDBURL:$InfluxDBPort/api/v2/write?org=$InfluxOrg&bucket=$InfluxBucket&precision=s" -H "Authorization: Token $InfluxToken" --data-binary "cloudflare_analytics_country,country=$cfRequestsCC visits=$cfRequests $cfTimeStamp"
